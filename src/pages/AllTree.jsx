...
import LastChild from "./ChildComponent";
...
const InnerTree = (props) => {
  ...
  <LastChild item={props.item}  setCollaps={setCollaps} collapse={collapse}/>
    ...
    >
      ...
      <>
      {
        ch.children ? <InnerTree  item={ch} key={ch.id}/> : (<TreeNode label={<LastChild  item={ch} />
        } />
        )
      }</>    
      )}
    ...
    <Tree
      ...
      label={<>
          <div className="flex items-center justify-center">
              ...
              <span className="text-gray-800 font-semibold text-xl font-medium text-indigo-500">{treeItem.name}</span>
              <span className="mt-2 text-gray-600 ">{treeItem.description}</span>
              </label>
              </form>
          </div>
      </>}
    >
      {treeItem.children ? treeItem.children.map((item)=><InnerTree item={item} key={item.id}/> ): "<></>" }
    </Tree>
  </Container>
);
  }
